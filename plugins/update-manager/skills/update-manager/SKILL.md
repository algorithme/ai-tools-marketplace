---
description: Tier-by-tier dependency updater for Rust, Python, and Node — discover, apply, gate, PR.
---
...
/update-manager <ecosystem>              # rust | python | node | all
...
Argument parsing:
- First positional arg: `refresh` OR an ecosystem (`rust`, `python`, `node`, `all`).
...
3. For each candidate matching `manual-only`, fire `AskUserQuestion` with the package name and
   reason; only include if the user confirms.
4. Print a table: `package | old | new | tier`. For uv-managed subprojects, annotate the tier
   column as `minor (uv: patch≡minor)` when `patch` was requested, so the user can see that
   the same command ran regardless of the requested tier.
...
2. Check whether the playbook prescribes a per-advisory loop for this `(ecosystem, tier)`
   combination (currently: bun security; pnpm security when the resolved version is < 9). When
...
`rust → python → node`. Within each ecosystem, run the requested tiers in order
`security → patch → minor → major`. Each `(ecosystem, tier)` cycle completes its full
Apply → Fix → Format → Gate → Commit pipeline before the next one starts. All commits
land on the same branch; the PR is opened once, after all ecosystems are done.
